...
def select_trades_dbg(bars, rr1=1.8, rr2=5.0, be_trigger_r=1.2, lock_r=1.0, cooldown_bars=0,
    ...
    for i in range(n):
        ...
        if can_enter and cand_dir != 0:
            _lv = cand_zone.top + entry_buffer_pts if cand_dir == 1 else cand_zone.bot - entry_buffer_pts
            _dentro = (c <= _lv) if cand_dir == 1 else (c >= _lv)
            _STATS.append(1 if _dentro else 0)
