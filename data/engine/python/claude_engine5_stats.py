...
def select_trades_stats(bars, rr1=1.8, rr2=5.0, be_trigger_r=1.2, lock_r=1.0, cooldown_bars=0,
    ...
    for i in range(n):
        ...
        if can_enter:
            ...
            for z in zones:
                ...
                if (i - z.born) > zone_max_age: continue
                if z.dir == 1 and l <= z.top + entry_buffer_pts and l >= z.bot:
                    _tocadas.add(round(z.top + entry_buffer_pts, 2))
                if z.dir == -1 and h >= z.bot - entry_buffer_pts and h <= z.top:
                    _tocadas.add(round(z.bot - entry_buffer_pts, 2))
            if cand_dir != 0: _STATS.append(len(_tocadas))
