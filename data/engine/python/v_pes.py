...
def sel_pes(bars, entry_mode='aire', rr1=1.8, rr2=5.0, be_trigger_r=1.2, lock_r=1.0, cooldown_bars=0,
    ...
    for i in range(n):
        ...
        if trade_open and dir_ == 1 and not just_entered:
            ...
            if hit_sl:
                ...
                if PESIMISTA and l <= active_sl:
                    cur_trade.update(exit_bar=i, exit_time=t, exit_reason="FULL_LOSS",
                                     exit_price=active_sl, tp1_was_hit=False)
                    trades.append(cur_trade); trade_open = False; last_close_bar = i
                else:
                    be_moved = True
