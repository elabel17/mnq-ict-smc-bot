...
def select_trades_2s(bars, rr1=1.8, rr2=5.0, be_trigger_r=1.2, lock_r=1.0, cooldown_bars=0,
    ...
    for i in range(n):
        ...
        if can_enter:
            # Modelo EJECUTABLE: una limite por lado, en la zona mas cercana al
            # precio. Las dos quedan en lados opuestos, asi que no pueden
            # llenarse en el mismo tick -> sin riesgo de doble fill.
            _bL = _bS = None; _dL = _dS = None
            _ref = closes[i-1] if i > 0 else c
            for z in zones:
                if z.born == i or not z.cleared: continue
                if (i - z.born) > zone_max_age: continue
                if z.dir == 1:
                    _lv = z.top + entry_buffer_pts
                    if _ref <= _lv: continue
                    if _dL is None or (_ref - _lv) < _dL: _dL = _ref - _lv; _bL = z
                else:
                    _lv = z.bot - entry_buffer_pts
                    if _ref >= _lv: continue
                    if _dS is None or (_lv - _ref) < _dS: _dS = _lv - _ref; _bS = z
            for z in [x for x in (_bL, _bS) if x is not None]:
                if z.born == i or not z.cleared: continue
                if (i - z.born) > zone_max_age: continue
